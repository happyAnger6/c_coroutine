...
typedef int (*co_wq_func_t)(wait_queue_t *wait, unsigned mode, int flags, void *key);
...
typedef struct co_wq_head {
    co_spinlock_t spin_lock;
    struct list_head task_list;
} co_wq_head_t;

typedef struct co_wq_entry {
    uint32_t flag;
...
static inline void _add_wq(co_wq_head_t *head, co_wq_entry_t *new)
{
    list_add(&new->task_list, &head->task_list);
}

static inline void _add_wq_tail(co_wq_head_t *head, co_wq_entry_t *new)
{
